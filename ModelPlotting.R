...
for ( i in c(1:8,10:numel(species))){
  ...
  JDmodFiles = list.files(path=species[i],pattern="*Model_JDNTYr_5I.Rdata",
  ...
  for (j in 1:numel(JDmodFiles)){
    ...
    if (tempMod$geese$error==0){ # not all models converged, don't plot non-converged models
      ...
      YrInd = c(1,which(str_detect(names(tempMod$coefficients),"YrF")))
      ...
      if (~isempty(JDNTInd)){
        ...
        # IntBasis = cbind(NTBasis[,1]*JBasis,NTBasis[,2]*JBasis,NTBasis[,3]*JBasis)
        IntBasis = numeric()
        whichJD = 3600
        for (k in 1:3){
        ...
        # thisJD = which(thisSite$JulianDay==270)
        # RealFitInt = model.matrix(tempMod)[thisJD,JDNTInd]%*%coef(tempMod)[JDNTInd]
        
        # JDNTMat = c(1,92,184,276)*matrix(NTForPlotting,nrow=4,ncol=5000,byrow=TRUE)
        # IntBasis = mSpline(NTForPlotting,
        #                    df=9,
        #                    periodic=T)
        # matplot(NTForPlotting,IntBasis, type = "p", pch = '.', ylab = "y", main ="JD*NT Basis, JD=1, DF=9")
        # RealFitInt = IntBasis%*%(coef(tempMod)[JDNTInd])
          
        # IntAdjust = as.vector(IntBasis[276,]%*%coef(tempMod)[JDNTInd])
          IntAdjust = IntBasis%*%coef(tempMod)[JDNTInd]
        RealFitCenterNTInt<- RealFitNT+coef(tempMod)[1]+IntAdjust-mean(JxNT) # adjust offset
        ...
        NTIntcil<-inv.logit(cisNTInt[1,]) # lowerCI bound
        NTIntciu<-inv.logit(cisNTInt[2,]) # upper CI bound
        
        NTIntplotDF = data.frame(NTForPlotting,RealFitCenterNTInt)
        colnames(NTIntplotDF) = c("NormTime","Fit")
        
        NTInt = ggplot(NTIntplotDF, aes(NormTime, Fit),
        ) + geom_smooth(fill = "grey",
                        colour = "black",
                        aes(ymin=NTIntcil, ymax=1.05*NTIntciu),
                        stat ="identity"
        ) + labs(x = NULL,
                 y = "Probability"
        ) + coord_cartesian(xlim=c(-1,1)
        ...
        #                  periodic=T) # basis functions for smooth function
        # RealFitJ<- JBasis%*%(coef(tempMod)[JDInd]) # multiply basis functions by model coefficients to get values of spline at each X
        RealFitCenterJInt<- RealFitJ+coef(tempMod)[1]+(coef(tempMod)[JDNTInd]%*%TKTK)-mean(JxJD) # add interaction and adjust offset
        RealFitCenterJ<- inv.logit(RealFitCenterJ)
        JDayBootstrapFits<- (JBasis%*%t(JDayBootstrapCoefs))+coef(tempMod)[1] # get spread of spline values at each X based on distributions of each coefficient
        quant.func<- function(x){quantile(x, probs=c(0.025,0.975))}
        cisJ<-apply(JDayBootstrapFits, 1, quant.func)-mean(JxJD) # confidence interval of smooth function estimate
...
colnames(tempPerf) = c("Species","Site","PropGoodResid","R2")
save(tempPerf,file=paste(outDir,'/TemporalModelPerformance.Rdata',sep=""))

colnames(lunPerf) = c("Species","Site","PropGoodResid","R2")
save(lunPerf,file=paste(outDir,'/LunarModelPerformance.Rdata',sep=""))
