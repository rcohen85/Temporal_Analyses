...
#       while (dev.cur()>1) {dev.off()}
      ...
      if (!isempty(JDNTInd)){
        ...
        for (l in 1:length(IntDays)){
          ...
          NTIntCI = cbind(NTIntCI,inv.logit(t(cisNTInt))) # get lower and upper CI bounds
          
          NTIntplotDF = cbind(NTIntplotDF,RealFitCenterNTInt)
          
        }
        
        NTIntplotDF = as.data.frame(cbind(NTForPlotting,NTIntplotDF))
        colnames(NTIntplotDF) = c("NormTime","Fit1","Fit2","Fit3","Fit4")
        NTIntCI = as.data.frame(NTIntCI)
        colnames(NTIntCI) = c("Fit1Low","Fit1High","Fit2Low","Fit2High","Fit3Low","Fit3High","Fit4Low","Fit4High")
        
        NTInt = ggplot(NTIntplotDF,aes(x=NormTime)
        ) + geom_smooth(aes(y=Fit1,ymin=NTIntCI$Fit1Low, ymax=1.05*NTIntCI$Fit1High),
                        fill = "#33FFE0",
                        alpha=0.3,
                        colour = "#33FFE0",
                        stat ="identity"
        ) + geom_smooth(aes(y=Fit2,ymin=NTIntCI$Fit2Low, ymax=1.05*NTIntCI$Fit2High),
                        fill = "#333EFF",
                        alpha=0.3,
                        colour = "#333EFF",
                        stat ="identity"
        ) + geom_smooth(aes(y=Fit3,ymin=NTIntCI$Fit3Low, ymax=1.05*NTIntCI$Fit3High),
                        fill = "#D933FF",
                        alpha=0.3,
                        colour = "#D933FF",
                        stat ="identity"
        ) + geom_smooth(aes(y=Fit4,ymin=NTIntCI$Fit4Low, ymax=1.05*NTIntCI$Fit4High),
                        fill = "#FF3333",
                        alpha=0.3,
                        colour = "#FF3333",
                        stat ="identity"
