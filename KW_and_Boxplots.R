...
for (i in 1:length(datFileList)){
  ...
  colnames(moonBins) = c("Timestamp","Event","BeginPhase","PhaseDur")
  ind = 1
  ...
  moonBins$PhaseDur[1:dim(moonBins)[1]-1] = diff(as.numeric(moonBins[,1]),lag=1)/(60*5)
  ...
  clickProp = whichBin$cnt/as.numeric(moonBins$PhaseDur[dim(moonBins)[1]-1])
