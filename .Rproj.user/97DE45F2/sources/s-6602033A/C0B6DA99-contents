...
for ( i in 1:numel(species)){
  ...
  JDmodFiles = list.files(path=species[i],pattern="*Model_JDYrDP.Rdata",
                          full.names=TRUE,recursive=FALSE,include.dirs=FALSE,no..=TRUE)
  LunmodFiles = list.files(path=species[i],pattern="*Model_LunPhaseAlt.Rdata",
                           full.names=TRUE,recursive=FALSE,include.dirs=FALSE,no..=TRUE)
  ...
  for (j in 1:numel(JDmodFiles)){
    ...
    site = str_remove(LunmodFiles[j],paste(outDir,"/",CTname,"/",sep=""))
    sites = c(sites,str_remove(site,"_5minBin_Model_LunPhaseAlt.Rdata"))
    
    load(JDmodFiles[j]) # load JD + DP + Yr model
    load(LunmodFiles[j]) # load LunPhase + Alt model
    ...
    if (site=="HAT"){
      ...
      startInd = which(thisSite$NightBinTimes>=as.POSIXct('2017-05-01 00:00:00',format="%Y-%m-%d %H:%M:%S",tz="GMT"))
    ...
    JDInd = 2:4 # julian day
    DPInd = c(1,5:7) # day phase
    YrInd = c(1,8:9) # year
    MPInd = 2:3 # moon phase
    AltInd = 4:8 # moon altitude
    ...
    # MoonMagBootstrapCoefs<- BootstrapParameters2[,2:6]
    ...
    # JxMoonMag = model.matrix(lunMod)[,2:6]*coef(lunMod)[2:6]
    ...
    densDF$normDens = densDF$normDens/864#max(densDF$normDens) 
    ...
    AdjustedDPCoefs = inv.logit(AdjustedPhsCoefs$Coefficient)
    ...
    Phs = ggplot(AdjustedDPCoefs,aes(DayPhase,Coefficient)
    ...
    RealFitCenterMP<- RealFitMP-mean(JxMP)# adjust offset
    RealFitCenterMP = inv.logit(RealFitCenterMP)
    MPBootstrapFits<- PhaseBasis%*%t(MoonPhaseBootstrapCoefs) # get spread of spline values at each X based on distributions of each coefficient
    quant.func<- function(x){quantile(x, probs=c(0.025,0.975))}
    ...
    whichMP = histc(masterLun$MoonPhase,seq(0,1,0.01)) # Count up observations on each Julian day
    densDF$normDens = (whichMP$cnt-min(whichMP$cnt))[1:365] # set min to 0
    # normalize by max # obs possible (288 5-min bins per day, times 3 observations of each Julian day in the study period)
    densDF$normDens = densDF$normDens/864#max(densDF$normDens) 
    ...
    saveName = paste(outDir,'/',CTname,'/',sites[j],"_",int,"_GEEGLM_MoonPhasePlot_Probs.pdf",sep="")
    ggsave(saveName,device="pdf", width=2, scale=3, height=0.5, units="in",dpi=600)
    while (dev.cur()>1) {dev.off()}
    ...
    RealFitAlt<- AltBasis%*%coef(lunMod)[4:8] # multiply basis functions by model coefficients to get values of spline at each X
    ...
    saveName = paste(outDir,'/',CTname,'/',sites[j],"_",int,"_GEEGLM_MoonAltPlot_Probs.pdf",sep="")
    ggsave(saveName,device="pdf", width=2, scale=3, height=0.5, units="in",dpi=600)
    while (dev.cur()>1) {dev.off()}
    ...
    # MoonMagForPlotting<- seq(min(masterLun$MoonMag),max(masterLun$MoonMag), length=5000)
    # MagBasis<- mSpline(MoonMagForPlotting,
    #                    knots=c(-9.5,-6.5),
    #                    Boundary.knots=c(min(masterLun$MoonMag),max(masterLun$MoonMag)))
    # RealFitMag<- MagBasis%*%coef(lunMod)[2:6] # multiply basis functions by model coefficients to get values of spline at each X
    ...
    # )
    #
    # saveName = paste(outDir,'/',CTname,'/',sites[j],"_",int,"_GEEGLM_MoonMagPlot_Probs.pdf",sep="")
    # ggsave(saveName,device="pdf", width=2, scale=3, height=0.5, units="in",dpi=600)
    # while (dev.cur()>1) {dev.off()}
    #
    # LIDdens = ggplot(densDF)+geom_area(aes(LunFrac,normDens),
    ...
    # ) + theme(axis.line = element_line(size=0.2),
    #           panel.background = element_blank())
    # saveName = paste(outDir,'/',CTname,'/',sites[j],"_",int,"_GEEGLM_MoonPresPlot_Probs.pdf",sep="")
    # ggsave(saveName,device="pdf", width=2, scale=3, height=0.75, units="in",dpi=600)
    # while (dev.cur()>1) {dev.off()}
    #
